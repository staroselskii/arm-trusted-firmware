...
1.  It passes control to the Test Secure Payload to perform its
    ...
    interrupts in the `sel1_int_entry` field. The TSPD passes control to the TSP at
    ...
    `tsp_sel1_int_entry()`. The TSP has to preserve the callee saved general
...
The TSP implements an entrypoint (`tsp_sel1_int_entry()`) for handling Secure-EL1
...
4.  It sets the `ELR_EL3` system register to `tsp_sel1_int_entry` and sets the
    `SPSR_EL3.DAIF` bits in the secure CPU context. It sets `x0` to
    `TSP_HANDLE_SEL1_INT_AND_RETURN`. If the TSP was in the middle of handling a
...
The TSP issues an SMC with `TSP_HANDLED_S_EL1_INT` as the function identifier to
...
upon receiving an SMC with `TSP_HANDLED_S_EL1_INT` and `TSP_PREEMPTED` as the
...
2.  If the function identifier is `TSP_HANDLED_S_EL1_INT`, it restores the
...
`tsp_sel1_int_entry()`.  The TSP handles the interrupt while ensuring that the
...
`TSP_HANDLED_S_EL1_INT` as the function identifier.
